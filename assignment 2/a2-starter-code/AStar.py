...
#         and if it is already on CLOSED, delete the new instance.
    #gs = g[S]
    #fs = g[S]+h(S)    # Save the cost of getting to S in a variable.
...
##          print("calculated="+str(g[new_state]+h(new_state))
##                +"new_f="+str(new_f)+str(new_state))
##          if new_state in OPEN:
##            print("oppen value="+str(OPEN[new_state]))
          if (new_f < g[new_state]+h(new_state)):
##            print(str(new_state)+": old="+str(g[new_state]+h(new_state))
##                  +";new="+str(new_f))
